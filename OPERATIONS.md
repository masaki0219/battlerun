...
## 5. UGC通報の確認と対応（App Store Review Guideline 1.2）

通報はアプリの `管理画面 → 通報キュー`（`contentReports`）へ非公開で届く。
投稿前フィルタとブロック機能だけでは運用責任を満たさないため、次の手順を継続する。

- **担当**: `role: admin` を持つ運用責任者。休日を含め、代行者を事前に決める。
- **確認頻度**: 毎日朝・夕の2回。新規通報は原則24時間以内に確認を開始し、まず「確認中」へ変更する。
- **優先対応**: 具体的な暴力予告、児童の安全、自傷他害、個人情報の暴露は最優先で確認し、必要に応じて対象の非表示・削除、アカウント制限、関係機関への連絡を行う。
- **通常対応**: 対象ドキュメントと前後の文脈をFirebase Consoleで照合する。違反なら対象を削除または安全な内容へ差し替え、必要に応じてFirebase Authenticationでアカウントを無効化して「対応済み」にする。違反が確認できなければ理由を記録したうえで「違反なし」にする。
- **証跡**: 通報ID、確認時刻、対象、判断、実施措置を運用記録へ残す。通報本文やUIDを公開Issueへ転載しない。
- **再発防止**: 新しい回避表記が確認できた場合は `lib/validation/bannedWords.ts` と `functions/src/bannedWords.ts` を同期更新し、ユニット・Rulesテスト後にFunctionsとRulesを再デプロイする。

アプリ上の「対応済み」は、対象コンテンツへの必要な措置と運用記録が完了した後にのみ選ぶ。
